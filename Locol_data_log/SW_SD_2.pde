...
#define DO_SENSOR 1	// Датчик растворенного в воде кислорода
#define PH_SENSOR 2	//Датчик pH
#define TEMP_SENSOR 3	//Датчик температуры воды
#define EC_SENSOR 4	//Датчик электропроводимости
#define TUR_SENSOR 5//Датчик мутности
...
void loop() {
  if ( millis()-lastT > 10000 )
  {
    SD_write_part(0,millis()/1000);
	  for ( int i = 1; i<6; i++)
	  {
      SD_write_part(i,getSensorData(i));
	  }
